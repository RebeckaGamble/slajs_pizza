...
const Checkout = () => {
  ...
  return (
    <section className="flex flex-col px-4 w-full max-w-[90rem]">
      <h2 className="font-semibold text-2xl py-6 text-center">Checkout</h2>
      <h3 className="font-semibold text-xl border-b pb-1">Your order </h3>
      <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 space-y-4 space-x-auto py-6">
        {order.map((item) => (
          <OrderItem
            key={item.id}
            id={item.id}
